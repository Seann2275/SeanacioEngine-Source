...
class AtlasFrameMaker extends FlxFramesCollection
{
	...
	*/
	...
	public static function construct2(key:String,?_excludeArray:Array<String> = null, ?noAntialiasing:Bool = false):FlxFramesCollection
	{
		// widthoffset = _widthoffset;
		// heightoffset = _heightoffset;
		...
		if (Paths.fileExists('characters/assets/$key/spritemap1.json', TEXT))
		{
			PlayState.instance.addTextToDebug("Only Spritemaps made with Adobe Animate 2018 are supported", FlxColor.RED);
			trace("Only Spritemaps made with Adobe Animate 2018 are supported");
			return null;
		...
		var animationData:AnimationData = Json.parse(Paths.getTextFromFile('characters/assets/$key/Animation.json'));
		var atlasData:AtlasData = Json.parse(Paths.getTextFromFile('characters/assets/$key/spritemap.json').replace("\uFEFF", ""));
